...
WidgetMetadata = {
  ...
  description: "哔哩哔哩高画质版（支持 4K / 1080P+）",
  author: "Minis",
  site: "https://www.bilibili.com",
  detailCacheDuration: 300,
  globalParams: [
    { name: "sessdata", title: "SESSDATA (推荐)", type: "input", value: "" },
    { name: "prefer_quality", title: "优先画质", type: "enumeration", value: "120",
      enumOptions: [
...
async function loadDetail(link){
  ...
  var sd="",pq=120;
  try{var s=Widget.storage.get("sessdata");if(s) sd=s;}catch(e){}
  try{var q=Widget.storage.get("prefer_quality");if(q) pq=parseInt(q,10)||120;}catch(e){}

  var ir=await Widget.http.get(API+"/x/web-interface/view?bvid="+encodeURIComponent(bvid),{headers:hd(sd)});
  ...
  var vu="";
  if(cid&&aid){
    try{var hp={bvid:bvid,cid:cid,qn:pq,fnval:4048,fnver:0,fourk:1,platform:"ios",high_quality:"1",video_codecid:"12",web_location:1315873};wbiSign(hp);var hr=await Widget.http.get(API+"/x/player/wbi/playurl"+qs(hp),{headers:hd(sd)});var hd_=hr&&hr.data;if(hd_&&hd_.code===0&&hd_.data&&hd_.data.durl) vu=hd_.data.durl[0].url;}catch(e){}
    try{var fr=await Widget.http.get(API+"/x/player/playurl?avid="+aid+"&cid="+cid+"&qn=80&platform=ios&otype=json&fourk=1",{headers:hd(sd)});var fd=fr&&fr.data;if(fd&&fd.code===0&&fd.data&&fd.data.durl) vu=vu||fd.data.durl[0].url;}catch(e){}
  }
  return{id:link,type:"video",title:v.title||bvid,link:link,posterPath:v.pic||undefined,videoUrl:vu||undefined,videoSources:vu?[{url:vu,type:"video/mp4",label:"HQ"}]:undefined,description:(v.desc||"")+"\nUP主: "+(v.owner?v.owner.name:""),rating:v.stat?v.stat.view:undefined,genreItems:(v.tid&&v.tname)?[{id:String(v.tid),title:v.tname}]:undefined,episodeItems:eps.length>1?eps:undefined,relatedItems:undefined,customHeaders:{Referer:"https://www.bilibili.com/"}};
}
